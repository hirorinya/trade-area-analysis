  ...
  try {
    ...
    if (process.env.MAPBOX_ACCESS_TOKEN) {
      try {
        ...
        console.warn('Mapbox Isochrone API failed, using fallback:', (error as Error).message);
        isochroneData = null;
      }
    }
  ...
  try {
    ...
    if (process.env.MAPBOX_ACCESS_TOKEN) {
      try {
        ...
        if (mapboxResponse.ok) {
          const mapboxData = await mapboxResponse.json() as any;
        ...
        console.warn('Mapbox Geocoding API failed, using fallback:', (error as Error).message);
        geocodeResult = null;
      }
    }
