...
class LocationModel {
    static async create(userId, locationData) {
        ...
        const projectCheck = await database_1.default.query('SELECT id FROM projects WHERE id = $1 AND user_id = $2', [project_id, userId]);
    ...
    static async findByProjectId(projectId, userId) {
        // Verify project belongs to user
        const projectCheck = await database_1.default.query('SELECT id FROM projects WHERE id = $1 AND user_id = $2', [projectId, userId]);
        if (projectCheck.rows.length === 0) {
            throw new Error('Project not found or access denied');
        }
        const query = `
    ...
    static async findNearby(latitude, longitude, radiusKm, userId) {
        const query = `
      SELECT l.id, l.project_id, l.name, l.address, 
             ST_X(l.coordinates) as longitude, 
             ST_Y(l.coordinates) as latitude,
    ...
    `;
        const result = await database_1.default.query(query, [longitude, latitude, userId, radiusKm * 1000]);
        return result.rows.map(location => ({
            ...location,
            coordinates: {
                type: 'Point',
